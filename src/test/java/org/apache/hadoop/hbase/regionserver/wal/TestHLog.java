...
public class TestHLog  {
  ...
  public void testWALWithEncryptionCodec() throws Exception {
    ...
    HLog log = new HLog(fs, hbaseDir, oldLogDir, ourGoodConf);
    try {
      long timestamp = System.currentTimeMillis();
      WALEdit cols = new WALEdit();
      ...
      log.completeCacheFlush(hri.getEncodedNameAsBytes(), tableName, logSeqId, false);
      log.close();
      final Path filename = log.computeFilename();
      log = null;

      // Open a reader on the log and check the write can be read back
  
      HLog.Reader reader = HLog.getReader(fs, filename, ourGoodConf);
      try {
        HLog.Entry entry = reader.next();
      ...
      Thread runner = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            HLog.Reader reader = HLog.getReader(fs, filename, ourBadConf);
            try {
              HLog.Entry entry = reader.next();
              assertEquals(columns, entry.getEdit().size());
              int idx = 0;
